# coding: utf-8
import json
import fancyimpute # library containing the implementations of the baseline imputation algos
import numpy as np
import pandas as pd
...
for ctx in open('json/json'):
    z = json.loads(ctx)['label'] # JSON to Py object using json.loads; pull out the labels from the unloaded object
    ctx = json.loads(ctx)['forward'] # JSON to Py object using json.loads; pull out the 'forward' entries from the unloaded object
    x = np.asarray(ctx['values']) # JSON to Py object using json.loads; pull out the 'values' from the unloaded object
    y = np.asarray(ctx['evals']) # JSON to Py object using json.loads; pull out the 'evals' from the unloaded object


    x_mask = np.asarray(ctx['masks']).astype(np.bool) # astype: Copy of the array, cast to a specified type.
    y_mask = np.asarray(ctx['eval_masks']).astype(np.bool) 

    x[~x_mask] = np.nan # the places where mask = 0 should be marked nan

    y[(~x_mask) & (~y_mask)] = np.nan # the places where mask = 0 and eval_mask = 0 should be marked nan
...
def get_loss(X, X_pred, Y):
    ...
    X_pred = np.nan_to_num(X_pred) # Replace NaN with zero and infinity with large finite numbers (default behaviour) or with the numbers defined by the user using the nan, posinf and/or neginf keywords.
    pred = X_pred[mask]
    label = Y[mask]
